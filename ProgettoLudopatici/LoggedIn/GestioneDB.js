...
function addUtente (oggettoUtente) {
    ...
    /*var operazione = store.add({
        "username": "greenv",                          // la variabile operazione dovrà essere gestita come la richiesta
        "password": "firstTry"                           //poiche open,add(aggiunta nuovo OGGETTO), put(modifica ogetto esistente,
...
function applicaModifiche(oggetto) {
    var trans = db.transaction(["utenti"],"readwrite");
    var store = trans.objectStore("utenti");
    var operazione = store.put(oggetto);
    operazione.onsuccess = function(e) {
    }
